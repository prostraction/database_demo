...
if ($_POST['configuration'] == 'new_configuration') {
	...
	$GLOBALS['disk_id'] = mysqli_insert_id($connectionDB);
	if ($GLOBALS['disk_id'] > 0) {
		
		$QueryComputer = "INSERT INTO computer (disk) VALUES ($GLOBALS['disk_id'])";
		$ExecQuery = mysqli_query($connectionDB, $QueryComputer);
		//$stmt_computer = mysqli_prepare($connectionDB, "INSERT INTO computer (disk) VALUES ($GLOBALS['disk_id'])");
		//mysqli_stmt_bind_param($stmt_computer, "s", $GLOBALS['disk_id']);
		//mysqli_stmt_execute($stmt_computer);
		$GLOBALS['computer_id'] = mysqli_insert_id($connectionDB);
	}
}
?> 
<?php
...
if (strlen($_POST['action']) > 0 ) {
	...
	if ($_POST['action'] == 'show_motherboard') {
		$Query = "SELECT model FROM computer INNER JOIN motherboard ON computer.motherboard = motherboard.id WHERE computer.id = $GLOBALS['computer_id'];";
	}
	else if ($_POST['action'] == 'show_cpu_fan') {
		$Query = "SELECT model FROM computer INNER JOIN cpu_fan ON computer.cpu_fan = cpu_fan.id WHERE computer.id = $GLOBALS['computer_id'];";
	}
	else if ($_POST['action'] == 'show_cpu') {
		$Query = "SELECT model FROM computer INNER JOIN cpu ON computer.cpu = cpu.id WHERE computer.id = $GLOBALS['computer_id'];";
	}
	else if ($_POST['action'] == 'show_psu') {
		$Query = "SELECT model FROM computer INNER JOIN psu ON computer.psu = psu.id WHERE computer.id = $GLOBALS['computer_id'];";
	}
	else if ($_POST['action'] == 'show_pc_case') {
		$Query = "SELECT model FROM computer INNER JOIN pc_case ON computer.pc_case = pc_case.id WHERE computer.id = $GLOBALS['computer_id'];";
	}
...
if (isset($_POST['update_motherboard'])) {
	...
	$Query = "UPDATE computer SET motherboard=$PostedValue WHERE computer.id = $GLOBALS['computer_id'];";
	$ExecQuery = mysqli_query($connectionDB, $Query);
}?>
...
if (isset($_POST['update_cpu'])) {
	...
	$Query = "UPDATE computer SET cpu=$PostedValue WHERE computer.id = $GLOBALS['computer_id'];";
	$ExecQuery = mysqli_query($connectionDB, $Query);
}?>
...
if (isset($_POST['update_cpu_fan'])) {
	...
	$Query = "UPDATE computer SET cpu_fan=$PostedValue WHERE computer.id = $GLOBALS['computer_id'];";
	$ExecQuery = mysqli_query($connectionDB, $Query);
}?>
...
if (isset($_POST['show_ram'])) {
	$Query = "SELECT id, model FROM ram_computer INNER JOIN ram ON ram_computer.ram_id = ram.id WHERE ram_computer.computer_id = $GLOBALS['computer_id']";
	$ExecQuery = mysqli_query($connectionDB, $Query);
	echo '<ul>';
	while ($Result = mysqli_fetch_array($ExecQuery)) {
...
if ($_POST['action'] == 'insert_ram') {
	...
	$string_arg1 = $GLOBALS['computer_id'];
...
if ($_POST['action'] == 'delete_ram') {
	...
	$Query = "DELETE FROM ram_computer WHERE ram_computer.computer_id=$GLOBALS['computer_id'] AND ram_computer.ram_id=$PostedValue;";	
    $ExecQuery = mysqli_query($connectionDB, $Query);
}?>
...
if (isset($_POST['show_gpu'])) {
	$Query = "SELECT id, model FROM gpu_computer INNER JOIN gpu ON gpu_computer.gpu_id = gpu.id WHERE gpu_computer.computer_id = $GLOBALS['computer_id']";
	$ExecQuery = mysqli_query($connectionDB, $Query);
	echo '<ul>';
	while ($Result = mysqli_fetch_array($ExecQuery)) {
...
if ($_POST['action'] == 'delete_gpu') {
	...
	$Query = "DELETE FROM gpu_computer WHERE gpu_computer.computer_id=$GLOBALS['computer_id'] AND gpu_computer.gpu_id=$PostedValue;";	
    $ExecQuery = mysqli_query($connectionDB, $Query);
}?>
...
if (isset($_POST['update_psu'])) {
	...
	$Query = "UPDATE computer SET psu=$PostedValue WHERE computer.id = $GLOBALS['computer_id'];";
	$ExecQuery = mysqli_query($connectionDB, $Query);
}?>
...
if (isset($_POST['update_pc_case'])) {
	...
	$Query = "UPDATE computer SET pc_case=$PostedValue WHERE computer.id = $GLOBALS['computer_id'];";
	$ExecQuery = mysqli_query($connectionDB, $Query);
}?>
